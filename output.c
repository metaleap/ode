...
void odeScreenClearRectText(OdeRect const* const rect) {
    for (UInt x = rect->pos.x, x_end = (rect->pos.x + rect->size.width); x < x_end;
         x += 1)
        for (UInt y = rect->pos.y, y_end = (rect->pos.y + rect->size.height); y < y_end;
             y += 1)
...
void odeRenderText(Str const text, OdeRect const* const screen_rect,
    ...
    if (clear_full_line)
        for (UInt x = screen_rect->pos.x + text.len;
             x < (screen_rect->pos.x + screen_rect->size.width); x += 1)
            ode.output.screen.prep[x][pos.y].rune.u32 = 0;
    for (UInt i = 0; i < text.len; i += 1)
        if (((pos.x + 2) >= (screen_rect->pos.x + screen_rect->size.width))
            || text.at[i] == '\n' || (i < (text.len - 1) && text.at[1 + i] == '\n')) {
            Str const ellipsis = str("…");
            ode.output.screen.prep[pos.x][pos.y].rune.bytes
                [strCopyTo((Str) {.at = ode.output.screen.prep[pos.x][pos.y].rune.bytes,
                                  .len = 0},
                           ellipsis)
                     .len] = 0;
            break;
...
OdeRect odeRender(OdeUiCtl* const ctl, OdeRect const screen_rect) {
    ...
    switch (ctl->dock) {
        case ode_uictl_dock_none: {
            dst_rect = rect(screen_rect.pos.x + ctl->rect.pos.x,
                            screen_rect.pos.y + ctl->rect.pos.y, ctl->rect.size.width,
        ...
        case ode_uictl_dock_left: {
            ...
            dst_rect = rect(screen_rect.pos.x, screen_rect.pos.y, width,
                            screen_rect.size.height);
            ret_rect = rect(screen_rect.pos.x + width, screen_rect.pos.y,
                            screen_rect.size.width - width, screen_rect.size.height);
        } break;
        case ode_uictl_dock_right: {
            UInt const width = uIntMin(ctl->rect.size.width, screen_rect.size.width);
            dst_rect = rect((screen_rect.pos.x + screen_rect.size.width) - width,
                            screen_rect.pos.y, width, screen_rect.size.height);
            ret_rect = rect(screen_rect.pos.x, screen_rect.pos.y,
                            screen_rect.size.width - width, screen_rect.size.height);
        } break;
        case ode_uictl_dock_top: {
            UInt const height = uIntMin(ctl->rect.size.height, screen_rect.size.height);
            dst_rect = rect(screen_rect.pos.x, screen_rect.pos.y,
                            screen_rect.size.width, height);
            ret_rect = rect(screen_rect.pos.x, screen_rect.pos.y + height,
                            screen_rect.size.width, screen_rect.size.height - height);
        } break;
        case ode_uictl_dock_bottom: {
            UInt const height = uIntMin(ctl->rect.size.height, screen_rect.size.height);
            dst_rect = rect(screen_rect.pos.x,
                            (screen_rect.pos.y + screen_rect.size.height) - height,
                            screen_rect.size.width, height);
            ret_rect = rect(screen_rect.pos.x, screen_rect.pos.y,
                            screen_rect.size.width, screen_rect.size.height - height);
        } break;
        default:
            odeDie(strZ(str2(NULL, str("BUG in render caller: invalid ctl->dock "),
                             uIntToStr(NULL, ctl->dock, 1, 10))),
                   false);
    }
...
#define ode_output_screen_buf_size                                                     \
    (64 * ode_output_screen_max_width * ode_output_screen_max_height)
    ...
    Bool const resized = (screen_size.width != ode_output_screen_size.width)
                         || (screen_size.height != ode_output_screen_size.height);
    if (resized) {
    ...
    if (!got_dirty_cells)
        for (UInt x = 0; x < screen_size.width; x += 1)
            for (UInt y = 0; y < screen_size.height; y += 1) {
                ...
                cells_dirty[x][y] = (prep->rune.u32 != real->rune.u32)
                                    || (prep->style != real->style)
                                    || (prep->color.bg != real->color.bg)
    ...
    if (got_dirty_cells) {
        ·ListOf(U8)
            buf = {.len = 0, .cap = ode_output_screen_buf_size, .at = &out_buf[0]};
#define ·out(·the·str)                                                                 \
    do {                                                                               \
        ...
        for (UInt y = 0; y < screen_size.height; y += 1)
            for (UInt x = 0; x < screen_size.width; x += 1)
                if (cells_dirty[x][y]) {
                    ...
                    // handle styles before colors, because here we might issue a
                    // reset-attrs
                    Bool styles_reset = false;
                    if (cell->style != state.style) {
                        static CStr ansis[97] = {
                            [ode_glyphstyle_none] = term_esc "0m",
                        ...
                        if (ansis[state.style] == NULL || ansis[state.style][0] == 0) {
                            ...
                            for (OdeGlyphStyleFlags st = ode_glyphstyle_bold;
                                 st <= ode_glyphstyle_overline; st += st)
                                if ((state.style & st) == st) {
                                    for (UInt i = ((ansi.len == 0) ? 0 : 2);
                                         ansis[st][i] != 'm'; i += 1)
                    ...
                    for (UInt i = 0; i < 3; i += 1) { // colors: 0=bg, 1=fg, 2=ul3
                        OdeRgbaColor** col_cur =
                            (i == 2) ? &state.color.ul3
                                     : (i == 1) ? &state.color.fg : &state.color.bg;
                        OdeRgbaColor* const col_cell =
                            (i == 2) ? cell->color.ul3
                                     : (i == 1) ? cell->color.fg : cell->color.bg;
                        if (styles_reset || col_cell != *col_cur) {
                            CStr const ansi_reset =
                                (i == 2)
                                    ? (term_esc "59m")
                                    : (i == 1) ? (term_esc "39m") : (term_esc "49m");
                            CStr const ansi_set =
                                (i == 2) ? (term_esc "58")
                                         : (i == 1) ? (term_esc "38") : (term_esc "48");
                            *col_cur = col_cell;
                            if (col_cell->rgba == 0)
                    ...
                    Str rune_str = (cell->rune.u32 == 0)
                                       ? ·len0(U8)
                                       : (Str) {.at = cell->rune.bytes, .len = 4};
                    if (rune_str.len != 0)
