    ...
    for (UInt i = 0; i < inputs.len; i += 1) {
        ...
        switch (input.kind) {
            ...
            case ode_input_mouse: {
                ...
                text.len += uintToBuf(&text.at[text.len], ode.input.mouse.pos.x, 3, 10, 0).len;
                ·push(text, ',');
                text.len += uintToBuf(&text.at[text.len], ode.input.mouse.pos.y, 3, 10, 0).len;
                if (input.of.mouse.scroll)
                    text = strPush(text, input.of.mouse.down ? ",Event.ScrollDown" : "Event.ScrollUp");
                if (input.of.mouse.did_move)
                    text = strPush(text, ",Event.Move");
                if (input.of.mouse.btn_l)
                    text = strPush(text, input.of.mouse.down
                                             ? ",Event.LBtnDown"
                                             : input.of.mouse.did_click ? ",Event.LBtnClick"
                                                                        : input.of.mouse.did_drop ? ",Event.LBtnDrop" : ",Event.LBtnDrag");
                if (input.of.mouse.btn_m)
                    text = strPush(text, input.of.mouse.down
                                             ? ",Event.MBtnDown"
                                             : input.of.mouse.did_click ? ",Event.MBtnClick"
                                                                        : input.of.mouse.did_drop ? ",Event.MBtnDrop" : ",Event.MBtnDrag");
                if (input.of.mouse.btn_r)
                    text = strPush(text, input.of.mouse.down
                                             ? ",Event.RBtnDown"
                                             : input.of.mouse.did_click ? ",Event.RBtnClick"
                                                                        : input.of.mouse.did_drop ? ",Event.RBtnDrop" : ",Event.RBtnDrag");
            ...
            case ode_input_hotkey: {
                ...
                OdeHotKey* const found = odeHotKey(input.of.key, input.mod_key.ctl, input.mod_key.alt, input.mod_key.shift);
                text = strPush(text, (found == NULL) ? "Unknown" : ((CStr)found->title.at));
            } break;
